...
function createDirectoryAndFile() {
  ...
  currentDir="$PWD"
  cd "$(dirname $proto)"
  command1="protoc -I=.  -I=$currentDir/specialist/src/proto -I=$currentDir/core/src/proto -I=$currentDir/parameter/src/proto -I=$currentDir/order/src/proto --swift_opt=Visibility=Public --swift_out=$logicPathToNewProto $(basename $proto)"
